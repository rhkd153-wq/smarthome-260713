...
window.SHData = (function () {
  ...
  /* =========================================================
   * 공간별 환경수정 체크리스트 (국내 현장 조사 기반)
   *  문스가 공간마다 하나씩 물어보고 → "예(해당돼요)"면 추천 도구를
   *  최종 솔루션(환경 안전·수정)에 묶는다. 임상적 의사결정 지원.
   *
   *  각 항목: { id, q(질문), tool(추천 도구), apply(적용 내용) }
   *   - 모든 질문은 "예 = 이 환경수정이 필요/도움" 방향으로 통일
   *  공간 키: entrance(현관) · bathroom(화장실) · bedroom(침실) · living(거실/부엌)
   * =======================================================*/
  var environmentChecklist = {
    entrance: [
      { id: 'ent-threshold', q: '현관 문턱이 높은가요?', tool: '발판 · 안전손잡이', apply: '문턱 옆에 발판과 손잡이를 설치해 넘기 쉽게 합니다.' },
      { id: 'ent-door', q: '현관문을 여닫기가 불편한가요?', tool: '레버형 문고리', apply: '돌리는 손잡이를 레버형으로 교체합니다.' },
      { id: 'ent-dark', q: '현관이 어두운가요?', tool: '센서등', apply: '사람이 오면 자동으로 켜지는 센서등을 설치합니다.' },
      { id: 'ent-slip', q: '현관 바닥이 미끄러운가요?', tool: '미끄럼 방지', apply: '미끄럼 방지 매트·테이프를 설치합니다.' },
      { id: 'ent-shoes', q: '신발을 신다가 넘어질 위험이 있나요?', tool: '벽걸이형 접이식 의자', apply: '현관 벽에 접이식 의자를 설치해 앉아서 신습니다.' },
      { id: 'ent-wheel', q: '휠체어로 드나들기 어려운가요?', tool: '경사로', apply: '1:12 기울기의 경사로(날개 포함)를 설치합니다.' }
    ],
    bathroom: [
      { id: 'bath-toilet', q: '변기에 접근이 어려운가요?', tool: '이동식 변기', apply: '야간 등 필요 시 이동식 변기를 활용합니다.' },
      { id: 'bath-slip', q: '욕실 바닥이 미끄러운가요?', tool: '미끄럼 방지 스프레이', apply: '바닥에 미끄럼 방지 처리를 합니다.' },
      { id: 'bath-dark', q: '화장실 조명이 어두운가요?', tool: 'LED 조명', apply: '밝은 LED 조명으로 교체합니다.' },
      { id: 'bath-storage', q: '수납장이 부족한가요?', tool: '수납장', apply: '손 닿는 위치에 수납장을 마련합니다.' },
      { id: 'bath-bar', q: '세면대 주변에 안전바가 필요한가요?', tool: '안전바', apply: '세면대 옆에 안전바를 설치합니다.' },
      { id: 'bath-shower', q: '샤워기 높이 조절이 안 되나요?', tool: '높이조절 샤워기', apply: '높이 조절이 되는 샤워기로 교체합니다.' },
      { id: 'bath-grab', q: '앉고 일어설 때 잡을 손잡이가 필요한가요?', tool: '안전손잡이', apply: '변기·샤워 공간에 안전손잡이를 설치합니다.' },
      { id: 'bath-chair', q: '높이 조절 목욕의자가 필요한가요?', tool: '목욕의자', apply: '앉아서 씻는 높이조절 목욕의자를 둡니다.' },
      { id: 'bath-urine', q: '소변이 잦아 불편한가요?', tool: '라이프스타일 수정', apply: '18시까지 수분 2L 섭취, 이후에는 갈증 시에만 섭취하도록 조정합니다.' }
    ],
    bedroom: [
      { id: 'bed-standrail', q: '앉았다 일어설 때 불안한가요?', tool: '세로형 안전손잡이', apply: '침대 옆에 세로형 안전손잡이를 고정합니다.' },
      { id: 'bed-riserail', q: '누웠다 일어날 때 잡을 곳이 부족한가요?', tool: '가로형 안전손잡이', apply: '침대 옆에 가로형 안전손잡이를 설치합니다.' },
      { id: 'bed-nightlight', q: '야간 조명이 부족한가요?', tool: '스탠드 · 야간등', apply: '침대 옆 스탠드나 콘센트형 야간등을 둡니다.' },
      { id: 'bed-cloth', q: '옷·물건 위치를 기억하기 어려운가요?', tool: '언어 피드백 스피커', apply: '음성으로 알려주는 스피커로 위치·정리를 돕습니다.' },
      { id: 'bed-storage', q: '옷·물건 수납 공간이 부족한가요?', tool: '서랍장', apply: '손 닿는 높이의 서랍장을 마련합니다.' },
      { id: 'bed-kit', q: '손 닿는 곳에 비상 연락처·약통·물품이 없나요?', tool: '도구상자', apply: '머리맡에 비상 도구상자를 비치합니다.' },
      { id: 'bed-chair', q: '잠시 앉아 쉴 의자가 부족한가요?', tool: '간이 의자', apply: '동선 중간에 간이 의자를 둡니다.' }
    ],
    living: [
      { id: 'liv-drag', q: '걸을 때 다리를 끌지는 않나요?', tool: 'AFO · 네발지팡이(4 cane)', apply: '발목 처짐을 보상하는 보조기·지팡이를 사용합니다.' },
      { id: 'liv-reach', q: '이동할 때 손으로 잡을 곳이 부족한가요?', tool: '안전손잡이', apply: '이동 동선(특히 우측)에 손잡이를 설치합니다.' },
      { id: 'liv-footmark', q: '발 디딜 위치가 불안한가요?', tool: '발바닥 모양 스티커', apply: '바닥에 발 위치 유도 스티커를 붙입니다.' },
      { id: 'liv-switch', q: '스위치 위치가 불편한가요? (예: 먼 곳·반대편)', tool: '리모컨형 LED 전등', apply: '리모컨으로 켜는 LED 전등으로 바꿉니다.' },
      { id: 'liv-handle', q: '집 안 문고리가 돌리는 원형인가요?', tool: '레버형 문고리', apply: '레버형 문고리로 교체합니다.' },
      { id: 'liv-fall', q: '낙상 위험이 있나요?', tool: '안전손잡이', apply: '위험 구간에 안전손잡이를 설치합니다.' },
      { id: 'liv-wire', q: '바닥에 전선이 어지럽게 있나요?', tool: '전선 정리', apply: '전선을 벽으로 고정·정리합니다.' },
      { id: 'liv-cane', q: '집 안 거동이 어려운가요?', tool: '실내용 지팡이', apply: '실내 이동용 지팡이를 사용합니다.' },
      { id: 'liv-window', q: '창문(미닫이)을 열기 어려운가요?', tool: '창문 보조 손잡이', apply: '창문에 보조 손잡이를 부착합니다.' },
      { id: 'liv-sink', q: '싱크대 높이가 맞지 않나요?', tool: '보조 발판', apply: '높이를 맞추는 보조 발판을 둡니다.' },
      { id: 'liv-gas', q: '가스 차단기가 없나요?', tool: '가스 차단기', apply: '설치 업체를 통해 가스 차단기를 설치합니다.' },
      { id: 'liv-table', q: '식탁까지 이동 거리가 먼가요?', tool: '가구 재배치', apply: '동선을 줄이도록 가구를 재배치합니다.' }
    ]
  };
